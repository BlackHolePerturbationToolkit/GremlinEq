//---------------------------------------------------------------------------
//
// $Id: RRGW.cc,v 1.7 2018/08/03 00:16:03 sahughes Exp $
//
//---------------------------------------------------------------------------
//
...
void RRGW::Wave(const int m, const Real t_ret,
		...
		const Real w, const Complex ZI,
		Real & hplus, Real & hcross)
{
  const Complex h = -2*S*ZI*exp(II*(m*phi - w*t_ret))/(w*w);
...
void RRGW::Wave(const int m, const int k,
                ...
                Real & hplus, Real & hcross)
{
  const Complex h = -2*S*ZI*exp(II*(m*phi - 2.*M_PI*(m*N_m + k*N_k)))/(w*w);

  hplus  = h.real();  hcross = -h.imag();
}

// Calculates (r times) Psi4.  To speed things up,
// calculate S(ct) in the calling routine.
// This is general, but doesn't really do inspirals well.
void RRGW::Psi4(const int m, const Real t_ret,
		const Real phi, const Real S,
		const Real w, const Complex ZI,
		Complex & psi4)
{
  psi4 = S*ZI*exp(II*(m*phi - w*t_ret));
}
